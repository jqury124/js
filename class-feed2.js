(function() {
    ...
    if (!myNick) {
        myNick = "Student " + Math.floor(Math.random() * 9000 + 1000); // Örn: Student 4821
    ...
    const styles = `
        ...
        .stream-body {
            ...
            display: flex;
            flex-direction: column;
            gap: 12px; /* Mesajlar arası boşluğu biraz açtık isimler rahat görünsün diye */
            scrollbar-width: none;
            
            /* METİN SEÇİMİ YASAKLANDI (Kopyalama engeli) */
            user-select: none;
    ...
    function initSystem() {
        ...
        feedArea.addEventListener('contextmenu', (e) => {
            e.preventDefault();  
        });
        feedArea.addEventListener('copy', (e) => {
            e.preventDefault();  
        });
